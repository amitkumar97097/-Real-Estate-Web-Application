...
export default function AddProperty() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setMsg("❌ Failed: " + (error.response?.data?.error || error.message || "Unknown error"));
    }
  };

  return (
    <form onSubmit={submit} className="grid gap-3 max-w-2xl p-4 border rounded-xl shadow">
      <h1 className="text-2xl font-semibold">Add Property Listing</h1>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <input
          ...
          value={payload.area}
          onChange={(e) => setPayload({ ...payload, area: Number(e.target.value) })}
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <input
          ...
          onChange={(e) => setPayload({ ...payload, bedrooms: Number(e.target.value) })}
        />
        <input
          className="border p-2 rounded"
          type="number"
          ...
          onChange={(e) => setPayload({ ...payload, bathrooms: Number(e.target.value) })}
        />
        <input
          className="border p-2 rounded"
